...
class Metrics:
    ...
    def area(self, drone1, drone2, drone3, simulation=False):
        area_array = np.array([])
        if simulation:
            drone1_time = drone1.timesp;
            drone1_pose = drone1.sp;
            drone2_pose = drone2.sp;
            drone3_pose = drone3.sp;
        else:
            drone1_time = drone1.time;
            drone1_pose = drone1.pose;
            drone2_pose = drone2.pose;
            drone3_pose = drone3.pose;
...
class Tools:
    ...
    def savedata(self, data):
        ...
        ws.write(0, 0, 'S_mean_er');
        ws.write(0, 1, data.S_mean_er)
        ws.write(1, 0, 'S_std_er');
        ws.write(1, 1, data.S_std_er)
        ws.write(2, 0, 'S_max_er');
        ws.write(2, 1, data.S_max_er)
