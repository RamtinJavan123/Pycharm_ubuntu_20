...
num_random_obstacles = 0  # number of random circular obstacles on the map
...
if random_obstacles:
    obstacles_poses = np.random.uniform(low=-2.5, high=2.5,
                                        size=(num_random_obstacles, 2))  # randomly located obstacles
    obstacles_goal_poses = np.random.uniform(low=-1.3, high=1.3,
                                             size=(num_random_obstacles, 2))  # randomly located obstacles goal poses
else:
    obstacles_poses = np.array(
        [[-2, 1], [1.5, 0.5], [-1.0, 1.5], [0.1, 0.1], [1, -2], [-1.8, -1.8]])  # 2D - coordinates [m]
...
if interactive:
    ...
    def human_pos_callback(data):
        ...
        q = np.array([data.transform.rotation.x, data.transform.rotation.y, data.transform.rotation.z,
...
centroid_route = [sum([p[0] for p in robots_poses]) / len(robots_poses),
                  sum([p[1] for p in robots_poses]) / len(robots_poses)]
...
with movie_writer.saving(fig, movie_file_name, max_its) if should_write_movie else get_dummy_context_mgr():
    for i in range(max_its):
        ...
        if interactive:
            ...
            direction = np.array(
                [cos(human_yaw), sin(human_yaw)])  # rotation of the swarm relatively to human orientation
        else:
        ...
        if impedance:
            ...
            imp_pose, imp_vel, imp_time_prev = velocity_imp(vels[0], imp_pose_prev, imp_vel_prev, imp_time_prev,
...
if postprocessing:
    ...
    plt.figure()
    plt.title("Leader-drone average velocity, <V>= %s m/s" % round(np.mean(np.array(norm_vels[0])), 2))
    for r in range(num_robots):
